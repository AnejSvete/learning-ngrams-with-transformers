...
# !DO NOT USE
class JelinekMercerEstimatorLearned(NgramsEstimator):
    ...
    def _fit(self, alphabet: Vocabulary, ngram_counts: NgramCounter):
        ...
        self.lms = {}
        for i in range(self.n):
            self.lms[i] = MLE(i, vocabulary=alphabet, counter=ngram_counts)
        self.lambdas = self._init_lambdas()

    def _init_lambdas(self):
        return torch.tensor(
            [(k + 1) ** 2 for k in range(self.n)],
            dtype=torch.float,
            requires_grad=True,
        )
    ...
    def pJM(self, word: str, context: List[str], k: int) -> float:
        if not self.frozen:
            lambdas = torch.nn.functional.softmax(self.lambdas, dim=0)
        else:
            lambdas = self.lambdas
        if k == 0:
            return lambdas[k] * self.lms[k].unmasked_score(word, context) + (
                1 - lambdas[k]
            ) * 1 / len(self.vocab)
        else:
            return lambdas[k] * self.lms[k].unmasked_score(word, context) + (
    ...
    def freeze(self):
        ...
        self.lambdas = torch.nn.functional.softmax(self.lambdas, dim=0)
        self.lambdas = list(self.lambdas.detach().numpy())
        self.learn_lambdas = False
        self.frozen = True
