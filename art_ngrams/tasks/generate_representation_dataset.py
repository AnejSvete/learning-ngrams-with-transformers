...
def get_data_old(
    ...
    while True:
        ...
        possible_strings = set(
            pLM.sample(ceil(1.75 * (N_train + N_test)), to_string=True)
        )
        while len(possible_strings) < 1.5 * (N_train + N_test):
            possible_strings = possible_strings.union(
                set(pLM.sample((N_train + N_test), to_string=True))
            )
        ...
        D_train = pLM.sample(ceil(1.75 * (N_train + N_test)), to_string=True)
        _D_train = set(D_train)
        D_test = pLM.sample(ceil(1.75 * (N_train + N_test)), to_string=True)
        _D_test = set(D_test)

        while len(set(_D_train)) < 1.5 * (N_train + N_test):
            D_train += pLM.sample((N_train + N_test), to_string=True)
            _D_train = _D_train.union(set(D_train))
        while len(set(_D_test)) < 1.5 * (N_train + N_test):
            D_test += pLM.sample((N_train + N_test), to_string=True)
...
def generate_dataset(cfg: DictConfig):
    ...
    print(
        ...
        f"N_test = {N_test}, n = {n}, mean_length = {mean_length}, method = {method}, "
        f"D = {D}, rank = {rank}, seed"
    )
    ...
    data_dir = os.path.join(
        base_dir,
        f"n{n}_Nsy{N_sym}_Ntr{N_train}_ml{mean_length}_m{method}_r{rank}_D{D}_s{seed}/",
    )
